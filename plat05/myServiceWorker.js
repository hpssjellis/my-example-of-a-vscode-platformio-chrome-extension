...
async function myOpenSidePanel() {
  try {
    ...
    console.log("[SERVICE WORKER] Side panel behavior set to open on action click.");
  } catch (myError) {
...
chrome.runtime.onInstalled.addListener((details) => {
  console.log(`[SERVICE WORKER] Extension ${details.reason}`);
  myOpenSidePanel();
});

// Log when service worker starts
console.log("[SERVICE WORKER] Service worker initialized");