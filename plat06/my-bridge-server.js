...
async function myConvertTFJSToTFLite(myModelData, myModelName) {
    ...
    try {
        ...
        // Save weight files
        if (myModelData.weightData) {
            const myWeightBuffer = Buffer.from(myModelData.weightData, 'base64');
            myFs.writeFileSync(
                myPath.join(myTFJSDir, 'weights.bin'),
                myWeightBuffer
            );
            myLog(`Saved weights: ${myWeightBuffer.length} bytes`, 'INFO');
        }
