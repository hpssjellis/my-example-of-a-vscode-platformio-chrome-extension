...
const mySerialDisconnectUrl = "http://localhost:8080/serial-disconnect"; // <-- ADDED
...
function myInitialize() {
    ...
    myAddLog("Select PlatformIO folder and click 'Connect Serial' to begin", "info");
 ...
 */
async function myDisconnectSerial() { // <-- NOW ASYNC
    ...
    // 2. Request server to close the actual SerialPort (NEW LOGIC)
    if (mySerialConnected) {
        try {
...
function myStartSerialStream() {
    ...
    try {
        ...
        mySerialEventSource.onerror = function(error) {
            ...
            // Note: We don't call myDisconnectSerial here to avoid infinite loops if the server is down,
            // but we rely on the server side to clean up the port on the next connect/compile.
            // UI is left in connected state until user manually disconnects or next successful connect.
        };
    ...
    try {
        ...
        if (myResponse.ok) {
            ...
            if (myResult.output) {
                ...
                myAddLog(myResult.output, "info"); // <-- NO LONGER TRUNCATED
            }
        } else {
            const myResult = await myResponse.json();
...
async function myConvertToKeras() {
    ...
    myAddTFLiteLog("=== KERAS CONVERSION STARTED ===", "info");
    ...
    try {
        ...
        if (myResponse.ok) {
            ...
            myAddTFLiteLog("SUCCESS: Model converted to Keras .h5!", "success");
        ...
        myConvertButton.disabled = false;
        myConvertButton.textContent = "Convert to Keras (.h5)";
...
function myDownloadKeras() {
    ...
    myAddTFLiteLog("Open the myOutput folder to access your .h5 file", "info");
}
