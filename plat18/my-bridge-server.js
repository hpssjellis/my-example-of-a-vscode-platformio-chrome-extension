...
// --- NEW SYSTEM CHECK ROUTES ---

// Health Check: PlatformIO Core Version
myApp.get('/check-pio-version', async (myReq, myRes) => {
    myLog('=== PLATFORMIO VERSION CHECK REQUEST ===', 'INFO');
    try {
        // Run pio --version command
        const { stdout: myStdout } = await myExecPromise('pio --version', { maxBuffer: 1024 * 50 });
        
        // Extract version number for cleaner response
        const myVersionMatch = myStdout.match(/version\s+([\d.]+)/i);
        const myVersion = myVersionMatch ? myVersionMatch[1] : myStdout.trim();
        
        myLog(`PlatformIO Version: ${myVersion}`, 'SUCCESS');
        
        myRes.status(200).json({
            success: true,
            message: `PlatformIO Core is installed. Version: ${myVersion}`,
            version: myVersion,
            output: myStdout
        });
    } catch (error) {
        myLog(`PlatformIO check failed: ${error.message}`, 'ERROR');
        myRes.status(500).json({
            success: false,
            message: 'PlatformIO Core not found or failed to execute. Is it in your PATH?',
            error: error.message
        });
    }
});

// Health Check: TFLite Converter Version (in WSL venv)
myApp.get('/check-tflite-converter', async (myReq, myRes) => {
    myLog('=== TFLITE CONVERTER VERSION CHECK REQUEST ===', 'INFO');
    
    // WARNING: This path must match your venv location in WSL
    const myWslVenvActivatePath = '/home/jerteach/tf-env/bin/activate';
    
    // Command to activate venv, check version, and deactivate in a single bash shell
    const myCommand = `wsl bash -c "source ${myWslVenvActivatePath} && tensorflowjs_converter --version && deactivate"`;
    myLog(`Executing WSL command: ${myCommand}`, 'INFO');
    
    try {
        const { stdout: myStdout, stderr: myStderr } = await myExecPromise(myCommand, { maxBuffer: 1024 * 50 });
        
        // Extract version number (e.g., '3.18.0')
        const myVersionMatch = myStdout.match(/(\d+\.\d+\.\d+)/i);
        const myVersion = myVersionMatch ? myVersionMatch[1] : myStdout.trim();
        
        if (myStdout.toLowerCase().includes('command not found') || myStderr.toLowerCase().includes('error')) {
             throw new Error(`Tool not found in VENV. Check installation. STDERR: ${myStderr}`);
        }
        
        myLog(`TensorFlow.js Converter Version: ${myVersion}`, 'SUCCESS');
        
        myRes.status(200).json({
            success: true,
            message: `tensorflowjs_converter is accessible. Version: ${myVersion}`,
            version: myVersion,
            output: myStdout
        });
    } catch (error) {
        myLog(`TFLite Converter check failed: ${error.message}`, 'ERROR');
        myRes.status(500).json({
            success: false,
            message: 'TFLite Converter check failed. Check WSL, venv path, and dependencies.',
            error: error.message
        });
    }
});
